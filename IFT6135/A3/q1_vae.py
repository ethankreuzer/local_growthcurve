...
def log_likelihood_bernoulli(mu, target):
    ...
    *** note. ***
    ...
    log_prob = target * torch.log(mu) + (1 - target) * torch.log(1 - mu)

    ll_bernoulli = torch.sum(log_prob, dim=1)
...
def log_likelihood_normal(mu, logvar, z):
    ...
    *** note. ***
    ...
    log_prob = -0.5 * (math.log(2 * math.pi) + logvar + ((z - mu) ** 2) / var)
...
def log_mean_exp(y):
    ...
    *** note. ***
    ...
    a, _ = y.max(dim=1, keepdim=True)

    lme = a.squeeze() + torch.log(torch.mean(torch.exp(y - a), dim=1))
...
def kl_gaussian_gaussian_analytic(mu_q, logvar_q, mu_p, logvar_p):
    ...
    *** note. ***
    ...
    kl_elementwise = 0.5 * (logvar_p - logvar_q + (var_q + (mu_q - mu_p) ** 2) / var_p - 1)
    kl_gg = torch.sum(kl_elementwise, dim=1)
...
def kl_gaussian_gaussian_mc(mu_q, logvar_q, mu_p, logvar_p, num_samples=1):
    ...
    *** note. ***
    ...
    z = mu_q + torch.exp(0.5 * logvar_q) * epsilon

    log_q = -0.5 * (math.log(2 * math.pi) + logvar_q + ((z - mu_q) ** 2) / torch.exp(logvar_q))
    log_q = log_q.sum(dim=2) 

    log_p = -0.5 * (math.log(2 * math.pi) + logvar_p + ((z - mu_p) ** 2) / torch.exp(logvar_p))
    log_p = log_p.sum(dim=2)

    kl_samples = log_q - log_p 
    kl_mc = kl_samples.mean(dim=1) 
