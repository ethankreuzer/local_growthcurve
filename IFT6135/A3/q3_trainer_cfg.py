    ...
    def sample(self, labels=None, cfg_scale=3., n_steps=None, set_seed=False):
        ...
        with torch.no_grad():
            ...
            for t_ in tqdm(range(n_steps)):
                ...
                pred_eps_uncond = self.eps_model(z_t, None)
                pred_eps_cond = self.eps_model(z_t, labels)
                w = cfg_scale
                pred_eps = (1.0 + w) * pred_eps_cond - w * pred_eps_uncond
                ...
                x_t = (z_t - sigma_t * pred_eps) / alpha_t

                z_t = self.diffusion.p_sample(z_lambda_t=z_t,
                                          lambda_t=lambda_t,
